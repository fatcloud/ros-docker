...
# Some dirty hack: restart container kinect after 5 seconds
printf "$MSG_HEADER For some wierd reason, kinect node never work without starting it twice\n"
printf "$MSG_HEADER So we are restarting it within 5 seconds."
for i in `seq 1 5`;
do
    sleep 1
    printf "."
done
docker restart -t 5 kinect
